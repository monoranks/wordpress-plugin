 ...
 * details (fixes waiting, open issues, audit dates, links). $state: unconnected | draft | none | scored, plus the fields
 * Column::cell_data sets. No JavaScript: the card is CSS.
 *
 * @package MonoRanks
 */
...
// The class is prefixed like every other one here: a bare name such as "arrow" is claimed by other plugins' stylesheets
// (SlimStat draws a white tooltip pointer on .arrow::after), and their rules would decorate this one.
$monoranks_arrow = '<span class="mr-arrow" aria-hidden="true">&#8599;</span>';
?>
<div class="monoranks-cell">
<?php if ( 'unconnected' === $state ) : ?>
	<span class="mr-xs"><a href="<?php echo esc_url( $settings_url ); ?>"><?php esc_html_e( 'Connect MonoRanks', 'monoranks' ); ?></a></span>
<?php elseif ( 'draft' === $state ) : ?>
	<span class="mr-row" tabindex="0"><span class="mr-xs">&mdash;</span></span>
	<div class="mr-tip" role="tooltip">
		<div class="mr-empty"><?php esc_html_e( 'Drafts are not sent to MonoRanks. This page gets its scores after the first audit that follows publishing.', 'monoranks' ); ?></div>
	</div>
<?php elseif ( 'none' === $state ) : ?>
	<span class="mr-row" tabindex="0" aria-label="<?php echo esc_attr( $hint ); ?>"><?php echo Admin::ring( null, 'sm', __( 'Health', 'monoranks' ), 'under' ) . Admin::ring( null, 'sm', __( 'AEO', 'monoranks' ), 'under' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- the partial escapes ?></span>
	<div class="mr-tip" role="tooltip">
		<div class="mr-kv">
	...
	<div class="mr-tip" role="tooltip">
		<div class="mr-kv">
			...
			<?php if ( $fixes_ready > 0 ) : ?>
				<div class="mr-bad"><?php echo esc_html( sprintf( /* translators: %s: number */ _n( '%s ready to apply', '%s ready to apply', $fixes_ready, 'monoranks' ), Admin::n( $fixes_ready ) ) ); ?></div>
			<?php else : ?>
				<div class="mr-ok"><?php esc_html_e( 'Nothing waiting', 'monoranks' ); ?></div>
			<?php endif; ?>
			...
			<?php else : ?>
				<div class="mr-ok"><?php esc_html_e( 'No open issues', 'monoranks' ); ?></div>
			<?php endif; ?>
		...
		<div class="mr-foot">
			...
			<?php if ( $page_url ) : ?><a href="<?php echo esc_url( $page_url ); ?>" target="_blank" rel="noopener"><?php esc_html_e( 'Open in MonoRanks', 'monoranks' ); ?> <?php echo $monoranks_arrow; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static markup ?></a><?php endif; ?>
		</div>
	</div>
<?php endif; ?>
</div>

...
class MonoRanks_Api {
	...
	public static function post( $path, array $body, $timeout = 15 ) {
		...
		$res = wp_remote_post(
			...
			array(
				...
				'sslverify' => ! MonoRanks_Connection::local(),
			)
		);
		if ( is_wp_error( $res ) ) {
			MonoRanks_Connection::update( array( 'last_error' => $res->get_error_message() ) );
			return array( 'code' => 0, 'body' => null, 'error' => $res->get_error_message() );
		}
		$code = (int) wp_remote_retrieve_response_code( $res );
		$json = json_decode( (string) wp_remote_retrieve_body( $res ), true );
		if ( 401 === $code ) {
			MonoRanks_Connection::update( array( 'key_state' => 'revoked', 'last_error' => 'MonoRanks did not accept the key' ) );
		} elseif ( $code >= 200 && $code < 300 ) {
			MonoRanks_Connection::update( array( 'key_state' => 'ok', 'last_sent_at' => gmdate( 'c' ), 'last_error' => null ) );
		} else {
			MonoRanks_Connection::update( array( 'last_error' => is_array( $json ) && isset( $json['error'] ) ? (string) $json['error'] : 'MonoRanks answered ' . $code ) );
		}
	...
	public static function send_status() {
		return self::post( '/status', MonoRanks_Rest::status_payload() );
	}
}
